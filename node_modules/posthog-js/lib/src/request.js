...
var _sendBeacon = function (options) {
    ...
    try {
        ...
        if ((0, core_1.isArray)(options.data) && options.data.length > 1 && (estimatedSize !== null && estimatedSize !== void 0 ? estimatedSize : 0) > BEACON_SPLIT_FLOOR_BYTES) {
            var mid = Math.ceil(options.data.length / 2);
            _sendBeacon(__assign(__assign({}, options), { data: options.data.slice(0, mid) }));
            _sendBeacon(__assign(__assign({}, options), { data: options.data.slice(mid) }));
            return;
        }
